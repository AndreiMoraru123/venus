...
//
// Original: Copyright (c) 2018 Tristan Brindle (tcbrindle at gmail dot com)
// Modified by: me (Andrei Moraru)
...
constexpr auto &include_regex = R"(#include <(venus(?:/\w*)+.h(?:pp)?)>)";
...
private:
  ...
  std::string process_one(const fs::path &path) {
    ...
    std::for_each(std::sregex_iterator(text.begin(), text.end(), regex_),
                  std::sregex_iterator{}, [&](const auto &match) {
    ...
    std::regex pragma_once_regex(R"(#pragma once\s*)");
    text = std::regex_replace(text, pragma_once_regex, "");

    processed_paths_.push_back(path);
    return text;
  }

  static void process_replacements(std::string &str,
                                   std::deque<replacement> &replacements) {
    std::sort(
